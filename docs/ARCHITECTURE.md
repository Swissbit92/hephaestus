...
```archview
{
  "caption": "Who reads what, and why the two consumers get different amounts of a plugin.",
  "nodes": [
    {
      "id": "cc",
      "label": "Claude Code",
      "sub": "loads a plugin whole",
      "tech": "skills · commands · agents · hooks · MCP",
      "kind": "external"
    },
    {
      "id": "codex",
      "label": "Codex · Pi",
      "sub": "discovers skill directories only",
      "tech": "the portable subset",
      "kind": "external"
    },
    {
      "id": "dev",
      "label": "developer / agent",
      "sub": "runs the gates against the repo itself",
      "kind": "external"
    },
    {
      "id": "market",
      "label": "marketplace.json",
      "sub": "the catalog entry point",
      "tech": ".claude-plugin/",
      "kind": "store",
      "group": "REPO"
    },
    {
      "id": "gates",
      "label": "scripts/ + scripts/checks/",
      "sub": "the executable invariants",
      "tech": "seam · floor · mtime · undetermined",
      "kind": "module",
      "group": "REPO"
    },
    {
      "id": "plugins",
      "label": "plugins/<name>/",
      "sub": "six independently-versioned units",
      "tech": "crucible · forge-unity · 4 more",
      "kind": "module",
      "group": "REPO"
    },
    {
      ...
      "sub": "the only part that travels",
      "kind": "module",
      "group": "REPO"
    }
  ],
...
The vocabulary this document assumes. Added because `cms check` noticed the document was
using eight acronyms it never defined — a check earning its place on its own source file.

| Term | Means |
|------|-------|
| **Anchor score** | The deterministic repo-health number from `repo_metrics.py`. Trend it; never read a single run as a grade |
| **archview** | A fenced JSON block inside `ARCHITECTURE.md` that renders to a diagram, so there is one file to edit and one linter to satisfy |
| **CHECK** | The executable line on an invariant — what turns a written rule into an enforced one |
| **Evidence class** | An entry in `.crucible/evidence.json` mapping changed paths to the proof that change requires |
| **JSONL** | Newline-delimited JSON; the append-only format of `docs/predictions.jsonl` |
| **MCP** | Model Context Protocol — the server interface `sqlite-readonly` and `mcp-starter` implement |
| **PROSE → FALSIFIABLE → CHECK → ENFORCED** | The pipeline an invariant walks from a sentence someone wrote to a script that fails the build |
| **Seam** | The enforced boundary stopping a generic plugin naming a specific engine, employer or product |
| **Tier A / Tier B** | Generic (domain-free) versus engine-adapter plugins; the distinction the seam enforces |
