...
- [ ] **Point `loop-harness` at a real, *noisy* target** — the dogfood proved the machinery on a green suite + a synthetic fault; the unproven path is **sustained triage of real, recurring failures** (a suite/job that actually breaks over time). That target also resolves the research note's open questions (empirically-validated stop-conditions, ledger schema). No capital path; **never emit-signals / never live-Mongo / never merge**. Wait for a real target rather than manufacture one.
