...
- [x] **Truth gates in `qa-gatekeeper`** ✅ shipped, then **measured** 2026-08-02 — and the measurement is the deliverable, not the sections. Aggregate over k=3 runs: assertion-quality **3/3**, mock-provenance **4/6**, unreached-code **2/3**, swallowed-exceptions **3/6**. Read: *unreliable, not dead* — a materially weaker claim than "it works", and not on its own a case for replacing any of them with a script. **Caution recorded because it bit twice:** a single k=3 run is not evidence. Swallowed-exceptions read 0/3 and then 3/3 with nothing changed in between; a conclusion was drawn from the first and contradicted by the second. Raise k before concluding anything about a prose gate. **The metric is also biased low:** a scenario scores *"did it find the defect I seeded"*, so a reviewer finding a defect nobody seeded is counted as a miss. Six times while building these fixtures the gate rejected a supposedly-clean twin and was **right every time** — a hardcoded credential, an unhandled `None`, unvalidated money arithmetic. Read these rates as a floor on the gate's value, never a ceiling.
