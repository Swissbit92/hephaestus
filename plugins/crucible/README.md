...
| **curate** | command | Periodic maintenance pass — deterministic scripts first (skill_lint, cms triage + check, repo metrics, predictions, invariants), then five library-health dimensions, then a ranked backlog split by who can act. Cadence, not per-change. |
...
| **skill-craft** | skill | Build, distil and check skills — `author` (scaffold + coach against nine patterns), `distil` (generalise a session into a skill), `lint` (`scripts/skill_lint.py`: budget + cross-skill duplication). |
